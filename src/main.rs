...
use board::{
    defs::Board,
    fen::{
        fen_parse_castling_rights, fen_parse_colour, fen_parse_enpassant, fen_parse_pieces,
        fen_split_string,
    },
...
fn main() {
    ...
    for fen_string in fen_string_test_cases {
        ...
        let setup_result = board.fen_setup(Some(fen_string));
        match setup_result {
            Ok(_) => println!("fen string: {fen_string} setup passed"),
            Err(e) => println!("fen string {fen_string}\n Error occured: {e}"),
        }
        print_bb(board.bb_pieces[Sides::WHITE][Pieces::KING]);
    }
}
